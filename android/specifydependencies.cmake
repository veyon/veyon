...
foreach(line ${lines})
    ...
    if(NOT ${currentLib} MATCHES "libQt5.*" AND matched)
        find_file(ourlib-${currentLib} ${currentLib} HINTS ${OUTPUT_DIR} ${EXPORT_DIR} ${ANDROID_ADDITIONAL_FIND_ROOT_PATH} NO_DEFAULT_PATH PATH_SUFFIXES lib)

        if(ourlib-${currentLib})
            list(APPEND extralibs "${ourlib-${currentLib}}")
        else()
            message(STATUS "could not find ${currentLib} in ${OUTPUT_DIR} ${EXPORT_DIR}/lib/ ${ANDROID_ADDITIONAL_FIND_ROOT_PATH}")
        endif()
    endif()
endforeach()
...
endforeach()
foreach(plugin ${ANDROID_EXTRA_PLUGINS}) #now we check for folders with extra stuff
    list(APPEND extraplugins ${plugin})
endforeach()
if(extraplugins)
    string(REPLACE ";" "," plugins "${extraplugins}")
    set(extraplugins "\"android-extra-plugins\": \"${plugins}\",")
endif()

set(extraPrefixDirs "")
foreach(prefix ${ANDROID_ADDITIONAL_FIND_ROOT_PATH})
	if (extraPrefixDirs)
		set(extraPrefixDirs "${extraPrefixDirs}, \"${prefix}\"")
	else()
		set(extraPrefixDirs "\"${prefix}\"")
	endif()
endforeach()

set(extraprefixdirs "\"extraPrefixDirs\": [${extraPrefixDirs}],")
...
string(REPLACE "##EXTRAPREFIXDIRS##" "${extraprefixdirs}" NEWCONTENTS "${NEWCONTENTS}")
