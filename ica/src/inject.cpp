...
LRESULT CALLBACK SASWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	return ((INJDATA*)pData)->fnCallWindowProc(((INJDATA*)pData)->fnOldSASWndProc, hwnd, uMsg, wParam, lParam);		
}
...
int InjectCode ()
{
	...
	case 1:
	{
		...
		if (!pGetSASWndRemote)
			__leave;
		if (!WriteProcessMemory(hProcess, pGetSASWndRemote, (const void *) &GetSASWnd, size, &dwNumBytesCopied) || dwNumBytesCopied != size)
			__leave;
		...
		if (!pSASWinProcRemote)
			__leave;
		if (!WriteProcessMemory(hProcess, pSASWinProcRemote, (const void *) &SASWindowProc, size, &dwNumBytesCopied) || dwNumBytesCopied != size)
			__leave;
		...
		if (!pCodeRemote)
			__leave;
		if (!WriteProcessMemory(hProcess, pCodeRemote, (const void *) &InjectFunc, size, &dwNumBytesCopied) || dwNumBytesCopied != size)
			__leave;
...
int EjectCode ()
{
	...
	if (!WriteProcessMemory(hProcess, pCodeRemote, (const void *) &EjectFunc, size, &dwNumBytesCopied) || dwNumBytesCopied != size)
	{
		VirtualFreeEx(hProcess, pCodeRemote, 0, MEM_RELEASE);
		CloseHandle(hProcess);
		return 0;
