...
static const Feature::Uid uidTextMessage = Feature::Uid( "e75ae9c8-ac17-4d00-8f0d-019348346208" );
	...
	m_features()
{
	...
	m_features += Feature( Feature::Action, Feature::ScopeAll, uidTextMessage,
						   tr( "Text message" ), QString(),
						   tr( "Use this function to send a text message to all "
							   "users e.g. to assign them new tasks." ),
						   QIcon( ":/resources/dialog-information.png" ) );

	m_features += Feature( Feature::Action, Feature::ScopeAll, uidSnapshot,
						   tr( "Snapshot" ), QString(),
...
void FeatureManager::runMasterFeature( const Feature& feature, const ComputerControlInterfaceList& computerControlInterfaces )
{
