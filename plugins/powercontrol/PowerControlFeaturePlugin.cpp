											   ...
											   const ComputerControlInterfaceList& computerControlInterfaces )
{
	...
	{
		const auto shutdownTimeout = arguments.value( EnumHelper::itemName(Argument::ShutdownTimeout), 60 ).toInt();
	...
	if( feature == m_powerDownDelayedFeature )
	{
		...
		if( dialog.exec() )
		{
			return controlFeature( feature.uid(), Operation::Start,
								   { { EnumHelper::itemName(Argument::ShutdownTimeout), dialog.seconds() } },
								   computerControlInterfaces );
		}
