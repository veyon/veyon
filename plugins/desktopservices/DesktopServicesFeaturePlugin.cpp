														  ...
														  const ComputerControlInterfaceList& computerControlInterfaces )
{
	if( master.appWindow() )
	{
		auto dialog = VeyonCore::qmlCore().createObjectFromFile( QStringLiteral("qrc:/desktopservices/StartAppDialog.qml"),
																 master.appWindow(),
																 this );
		connect( this, &DesktopServicesFeaturePlugin::acceptStartAppDialog, dialog, // clazy:exclude=connect-non-signal
				 [this, &master, computerControlInterfaces]( const QString& app, const QString& saveItemName ) {
					 startApp( app, saveItemName, master, computerControlInterfaces );
				 } );
		return;
	}
												...
												const ComputerControlInterfaceList& computerControlInterfaces )
{
	if( master.appWindow() )
	{
		auto dialog = VeyonCore::qmlCore().createObjectFromFile( QStringLiteral("qrc:/desktopservices/OpenWebsiteDialog.qml"),
																 master.appWindow(),
																 this );
		connect( this, &DesktopServicesFeaturePlugin::acceptOpenWebsiteDialog, dialog, // clazy:exclude=connect-non-signal
				 [this, &master, computerControlInterfaces]( const QString& app, const QString& saveItemName ) {
					 openWebsite( app, saveItemName, master, computerControlInterfaces );
				 } );
		return;
	}
