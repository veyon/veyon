...
void KMultiTabBarInternal::paintEvent( QPaintEvent * _pe )
{
	QWidget::paintEvent( _pe );
	QPainter p( this );
	p.fillRect( 0, 0, width(), height(), QColor( 0, 96, 255 ) );
	p.drawPixmap( 0, height()-300, QPixmap( ":/resources/sidebar.png" ) );
	p.rotate( 270 );
	QFont f = p.font();
	f.setBold( TRUE );
	f.setPointSize( 15 );
	...
	p.drawText( -height()+32, 19, QString( "iTALC %1" ).arg( PACKAGE_VERSION ) );
}
...
{
	KMultiTabBarTab  *tab;
	m_tabs.append(tab= new KMultiTabBarTab(pic,text,desc,id,box,m_position,m_style));
...
KMultiTabBarButton::KMultiTabBarButton(const QPixmap& pic,const QString& text,
					const QString & desc,QMenu *popup,
	...
	m_description( desc )
{
	setIcon(QPixmap::fromImage( fastQImage( pic ).scaled( 16, 16 ) ) );
	setText(text);
	m_position=pos;
...
void KMultiTabBarButton::enterEvent( QEvent * _e )
{
	...
	toolButtonTip * tbt = new toolButtonTip( m_origIcon, m_text,
						m_description,
...
QSize KMultiTabBarButton::sizeHint() const
{
    ...
    int iw = 0, ih = 0;
    if ( !icon().isNull() ) {
        iw = icon().pixmap( 16, 16 ).width() + 4;
        ih = icon().pixmap( 16, 16 ).height();
        w += iw;
        h = qMax( h, ih );
    }
...
KMultiTabBarTab::KMultiTabBarTab(const QPixmap& pic, const QString& text,
	...
	:KMultiTabBarButton(pic,desc,text,0,id,parent,pos,style),
 	m_showActiveTabText(false)
{
	d=new KMultiTabBarTabPrivate();
	setIcon(QPixmap::fromImage( fastQImage( pic ).scaled( iconSize() ) ) );
	m_expandedSize=24;
	setCheckable(true);
}
...
int KMultiTabBar::appendButton(const QPixmap &pic ,int id,QMenu *popup,const QString&)
{
	...
	m_buttons.append(btn= new KMultiTabBarButton(pic,QString(),QString(),
...
int KMultiTabBar::appendTab(const QPixmap &pic ,int id ,const QString& text,
					const QString & desc)
{
 m_internal->appendTab(pic,id,text,desc);
 return 0;
}
